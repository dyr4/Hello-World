...
void encryptFile(const string& inputFileName, const string& outputFileName, const string& initialGamma) {
    ...
    // Вывод длины открытого текста
    cout << "Длина открытого текста: " << plainText.size() << endl;
    ...
    cout << "Шифрование завершено. Контрольная сумма исходного текста: " << static_cast<int>(checksum) << endl;
}
...
void decryptAndValidate(const string& inputFileName, const string& initialGamma) {
    ...
    cout << "Расшифровка успешна. Контрольная сумма: " << static_cast<int>(calculatedChecksum) << endl;
}
