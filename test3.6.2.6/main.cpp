...
void writeFile()
{
    ...
    cin >> outputFileName;
    string encode_Text = "Тест упешный";
    ofstream outputFile(outputFileName);
    if (outputFile.is_open())
    {
        outputFile << encode_Text;
        outputFile.close();
        cout<<"В файл успешно всё записано"<<endl;
    } else {
...
void testDay() {
    // Тесты для Day()
    assert(Day() >= 0 && Day() <= 6); // День недели должен быть в диапазоне от 0 до 6
}

void testGenerateGamma() {
    // Тесты для generateGamma()
